...
from src.neo4j_db.neo4j_graph import get_neo4j_schema
    ...
    def __call__(self, state: KylaState, config: RunnableConfig) -> KylaState:
        ...
        if type(last_message) is HumanMessage:
            last_message = HumanMessage(
                content=configurable.load_db_instruction.format(
            ...
            return {'load_db_messages': [response]}

        return {'load_db_messages': []}
